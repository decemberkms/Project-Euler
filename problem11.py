'''
In the 20×20 grid below, four grid_20 along a diagonal line have been marked in red.
...
The product of these grid_20 is 26 × 63 × 78 × 14 = 1788696.

What is the greatest product of four adjacent grid_20 in the same direction (up, down, left, right, or diagonally) in the 20×20 grid?
'''
...
grid_20 = grid_20.strip().split('\n')
grid_20 = [map(int,x.strip().split(' ')) for x in grid_20]
print(grid_20)
...
for i in range(len(grid_20)-3):
	for j in range(len(grid_20[0])-3):
		sub = []
		sub.append(grid_20[i][j:j+4])
		sub.append(grid_20[i+1][j:j+4])
		sub.append(grid_20[i+2][j:j+4])
		sub.append(grid_20[i+3][j:j+4])
		a.append(sub)
